...
public class AutoBone {
	...
	public final FastList<SkeletonConfigValue> lengthConfigs = new FastList<SkeletonConfigValue>(
			new SkeletonConfigValue[] { SkeletonConfigValue.HEAD, SkeletonConfigValue.NECK, SkeletonConfigValue.TORSO,
					SkeletonConfigValue.HIPS_WIDTH, SkeletonConfigValue.LEGS_LENGTH });
	...
	// TODO Needs much more work, probably going to rethink how the errors work to
	...
	// kneeLegRatio seems to be around 0.54 to 0.6 after asking a few people in the
	// SlimeVR discord.
	public float kneeLegRatio = 0.55f;
	// kneeLegRatio seems to be around 0.55 to 0.64 after asking a few people in the
	// SlimeVR discord. TODO : Chest should be a bit shorter (0.54?) if user has an
	...
	public AutoBone(VRServer server) {
		...
		this.offsetSlideErrorFactor = server.config.getFloat("autobone.offsetSlideErrorFactor",
		...
		this.proportionErrorFactor = server.config.getFloat("autobone.proportionErrorFactor",
				this.proportionErrorFactor);
		...
		this.positionOffsetErrorFactor = server.config.getFloat("autobone.positionOffsetErrorFactor",
	...
	public void reloadConfigValues(List<PoseFrameTracker> trackers) {
		...
		if (server.config.getBoolean("autobone.forceChestTracker", false) || (trackers != null
				&& TrackerUtils.findTrackerForBodyPosition(trackers, TrackerPosition.CHEST) != null)) {
		...
		if (server.config.getBoolean("autobone.forceHipTracker", false)
				|| (trackers != null && TrackerUtils.findTrackerForBodyPosition(trackers, TrackerPosition.HIP) != null
						&& TrackerUtils.findTrackerForBodyPosition(trackers, TrackerPosition.WAIST) != null)) {
	 ...
	 * {@link VRServer}
	 *
	 * @return The {@link HumanSkeleton} associated with the {@link VRServer}, or
	...
	public Float getConfig(SkeletonConfigValue config, Map<SkeletonConfigValue, Float> configs,
	...
	public float sumSelectConfigs(List<SkeletonConfigValue> selection, Map<SkeletonConfigValue, Float> configs,
			Map<SkeletonConfigValue, Float> configsAlt) {
		float sum = 0f;
	...
	public float processFrames(PoseFrames frames, boolean calcInitError, float targetHeight,
		...
		if (targetHeight < 0f) {
			...
			if (skeleton != null) {
				// If there is a skeleton available, calculate the target height from its
				// configs
				targetHeight = sumSelectConfigs(heightConfigs, skeleton.getSkeletonConfig());
				LogManager
						.warning("[AutoBone] Target height loaded from skeleton (Make sure you reset before running!): "
								+ targetHeight);
			} else {
				// Otherwise if there is no skeleton available, attempt to get the max HMD
		...
		for (int epoch = calcInitError ? -1 : 0; epoch < numEpochs; epoch++) {
			...
			// Iterate over the frames using a cursor and an offset for comparing frames a
			// certain number of frames apart
			for (int cursorOffset = minDataDistance; cursorOffset <= maxDataDistance
					&& cursorOffset < frameCount; cursorOffset++) {
				for (int frameCursor = 0; frameCursor < frameCount - cursorOffset; frameCursor += cursorIncrement) {
					...
					float errorDeriv = getErrorDeriv(frames, frameCursor, frameCursor2, skeleton1, skeleton2,
							targetHeight - curHeight, 1f);
					...
					for (Entry<SkeletonConfigValue, Float> entry : configs.entrySet()) {
						...
						for (int i = 0; i < 2; i++) {
							...
							// float newScaleLength = isLengthVar ? scaleLength + curAdjustVal :
							// scaleLength;
							float newErrorDeriv = getErrorDeriv(frames, frameCursor, frameCursor2, skeleton1, skeleton2,
									targetHeight - newHeight, 1f);
		...
		LogManager.info("[AutoBone] Target height: " + targetHeight + " New height: " + finalHeight);
	...
	protected float getSlideErrorDeriv(PoseFrameSkeleton skeleton1, PoseFrameSkeleton skeleton2) {
		...
		// Divide by 4 to halve and average, it's halved because you want to approach a
	...
	protected float getOffsetSlideErrorDeriv(PoseFrameSkeleton skeleton1, PoseFrameSkeleton skeleton2) {
		...
		// Divide by 12 to halve and average, it's halved because you want to approach a
		// midpoint, not the other point
	...
	// point over time
	protected float getPositionOffsetErrorDeriv(PoseFrames frames, int cursor1, int cursor2,
			PoseFrameSkeleton skeleton1, PoseFrameSkeleton skeleton2) {
		float offset = 0f;
		int offsetCount = 0;
	...
	protected float getErrorDeriv(PoseFrames frames, int cursor1, int cursor2, PoseFrameSkeleton skeleton1,
			PoseFrameSkeleton skeleton2, float heightChange, float distScale) {
		float totalError = 0f;
		float sumWeight = 0f;

		if (slideErrorFactor > 0f) {
			// This is the main error function, this calculates the distance between the
		...
		if (offsetSlideErrorFactor > 0f) {
			// This error function compares the distance between the feet on each frame and
		...
		if (proportionErrorFactor > 0f) {
			// This error function compares the current values to general expected
		...
		if (heightErrorFactor > 0f) {
			// This error function compares the height change to the actual measured height
			// of the headset
		...
		if (positionErrorFactor > 0f) {
			// This error function compares the position of an assigned tracker with the
			// position on the skeleton
			totalError += (getPositionErrorDeriv(frames, cursor1, skeleton1)
					+ getPositionErrorDeriv(frames, cursor2, skeleton2) / 2f) * distScale * positionErrorFactor;
			sumWeight += positionErrorFactor;
		}

		if (positionOffsetErrorFactor > 0f) {
			// This error function compares the offset of the position of an assigned
			// tracker with the position on the skeleton
			totalError += getPositionOffsetErrorDeriv(frames, cursor1, cursor2, skeleton1, skeleton2) * distScale
	...
	protected void updateSkeletonBoneLength(PoseFrameSkeleton skeleton1, PoseFrameSkeleton skeleton2,
			SkeletonConfigValue config, float newLength) {
	...
	public void saveRecording(PoseFrames frames) {
		if (saveDir.isDirectory() || saveDir.mkdirs()) {
			...
			LogManager.info("[AutoBone] Exporting frames to \"" + saveRecording.getPath() + "\"...");
			if (PoseFrameIO.writeToFile(saveRecording, frames)) {
				LogManager.info(
						"[AutoBone] Done exporting! Recording can be found at \"" + saveRecording.getPath() + "\".");
			} else {
				LogManager.severe("[AutoBone] Failed to export the recording to \"" + saveRecording.getPath() + "\".");
			}
		} else {
	...
	public List<Pair<String, PoseFrames>> loadRecordings() {
		...
		if (loadDir.isDirectory()) {
			...
			if (files != null) {
				for (File file : files) {
					if (file.isFile()
							&& org.apache.commons.lang3.StringUtils.endsWithIgnoreCase(file.getName(), ".pfr")) {
						LogManager
								.info("[AutoBone] Detected recording at \"" + file.getPath() + "\", loading frames...");
						PoseFrames frames = PoseFrameIO.readFromFile(file);

						if (frames == null) {
							LogManager.severe("Reading frames from \"" + file.getPath() + "\" failed...");
						} else {
