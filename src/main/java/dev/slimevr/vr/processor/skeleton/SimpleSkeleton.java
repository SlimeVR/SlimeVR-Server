...
public class SimpleSkeleton extends HumanSkeleton implements SkeletonConfigCallback {
	...
	protected SimpleSkeleton(List<? extends ComputedHumanPoseTracker> computedTrackers) {
		...
		// Set default skeleton configuration (callback automatically sets initial
		// offsets)
	...
	public SimpleSkeleton(List<? extends Tracker> trackers, List<? extends ComputedHumanPoseTracker> computedTrackers,
			Map<SkeletonConfigValue, Float> configs, Map<SkeletonConfigValue, Float> altConfigs) {
		// Initialize
	...
	public void setTrackersFromList(List<? extends Tracker> trackers, boolean setHmd) {
		...
		this.neckTracker = TrackerUtils.findTrackerForBodyPositionOrEmpty(trackers, TrackerPosition.NECK,
				TrackerPosition.HMD, null);
		this.chestTracker = TrackerUtils.findTrackerForBodyPositionOrEmpty(trackers, TrackerPosition.CHEST,
				TrackerPosition.WAIST, TrackerPosition.HIP);
		this.waistTracker = TrackerUtils.findTrackerForBodyPositionOrEmpty(trackers, TrackerPosition.WAIST,
				TrackerPosition.HIP, TrackerPosition.CHEST);
		this.hipTracker = TrackerUtils.findTrackerForBodyPositionOrEmpty(trackers, TrackerPosition.HIP,
				TrackerPosition.WAIST, TrackerPosition.CHEST);

		this.leftKneeTracker = TrackerUtils.findTrackerForBodyPositionOrEmpty(trackers, TrackerPosition.LEFT_KNEE,
				TrackerPosition.LEFT_ANKLE, null);
		this.leftAnkleTracker = TrackerUtils.findTrackerForBodyPositionOrEmpty(trackers, TrackerPosition.LEFT_ANKLE,
				TrackerPosition.LEFT_KNEE, null);
		this.leftFootTracker = TrackerUtils.findTrackerForBodyPosition(trackers, TrackerPosition.LEFT_FOOT);

		this.rightKneeTracker = TrackerUtils.findTrackerForBodyPositionOrEmpty(trackers, TrackerPosition.RIGHT_KNEE,
				TrackerPosition.RIGHT_ANKLE, null);
		this.rightAnkleTracker = TrackerUtils.findTrackerForBodyPositionOrEmpty(trackers, TrackerPosition.RIGHT_ANKLE,
				TrackerPosition.RIGHT_KNEE, null);
		...
		this.rightControllerTracker = TrackerUtils.findTrackerForBodyPosition(trackers,
				TrackerPosition.RIGHT_CONTROLLER);
		this.leftForearmTracker = TrackerUtils.findTrackerForBodyPositionOrEmpty(trackers, TrackerPosition.LEFT_FOREARM,
				TrackerPosition.LEFT_UPPER_ARM, null);
		this.rightForearmTracker = TrackerUtils.findTrackerForBodyPositionOrEmpty(trackers,
				TrackerPosition.RIGHT_FOREARM, TrackerPosition.RIGHT_UPPER_ARM, null);
		this.leftUpperArmTracker = TrackerUtils.findTrackerForBodyPositionOrEmpty(trackers,
				TrackerPosition.LEFT_UPPER_ARM, TrackerPosition.LEFT_FOREARM, null);
		this.rightUpperArmTracker = TrackerUtils.findTrackerForBodyPositionOrEmpty(trackers,
				TrackerPosition.RIGHT_UPPER_ARM, TrackerPosition.RIGHT_FOREARM, null);
	...
	public void setComputedTrackersAndFillNull(List<? extends ComputedHumanPoseTracker> trackers,
	...
	public void fillNullComputedTrackers(boolean onlyFillWaistAndFeet) {
		if (computedWaistTracker == null) {
			computedWaistTracker = new ComputedHumanPoseTracker(Tracker.getNextLocalTrackerId(),
		...
		if (computedLeftFootTracker == null) {
			computedLeftFootTracker = new ComputedHumanPoseTracker(Tracker.getNextLocalTrackerId(),
		...
		if (computedRightFootTracker == null) {
			computedRightFootTracker = new ComputedHumanPoseTracker(Tracker.getNextLocalTrackerId(),
		...
		if (!onlyFillWaistAndFeet) {
			if (computedChestTracker == null) {
				computedChestTracker = new ComputedHumanPoseTracker(Tracker.getNextLocalTrackerId(),
			...
			if (computedLeftKneeTracker == null) {
				computedLeftKneeTracker = new ComputedHumanPoseTracker(Tracker.getNextLocalTrackerId(),
			...
			if (computedRightKneeTracker == null) {
				computedRightKneeTracker = new ComputedHumanPoseTracker(Tracker.getNextLocalTrackerId(),
			...
			if (computedLeftElbowTracker == null) {
				computedLeftElbowTracker = new ComputedHumanPoseTracker(Tracker.getNextLocalTrackerId(),
			...
			if (computedRightElbowTracker == null) {
				computedRightElbowTracker = new ComputedHumanPoseTracker(Tracker.getNextLocalTrackerId(),
			...
			if (computedLeftHandTracker == null) {
				computedLeftHandTracker = new ComputedHumanPoseTracker(Tracker.getNextLocalTrackerId(),
			...
			if (computedRightHandTracker == null) {
				computedRightHandTracker = new ComputedHumanPoseTracker(Tracker.getNextLocalTrackerId(),
	...
	protected void updateLocalTransforms() {
		...
		if (extendedKneeModel)
			calculateKneeLimits(rotBuf1, rotBuf2, leftKneeTracker.getConfidenceLevel(),
		...
		if (extendedKneeModel)
			calculateKneeLimits(rotBuf1, rotBuf2, rightKneeTracker.getConfidenceLevel(),
		...
		if (extendedPelvisModel && hasKneeTracker) {
			...
			// trackerWaistNode.localTransform.setRotation(rotBuf2); // <== Provides cursed
			// results from my test in VRChat when sitting or laying down -Erimel
			// TODO : Correct the trackerWaistNode without getting cursed results (only
			// correct yaw?)
			// TODO : Use vectors to add like 50% of waist tracker yaw to waist node to
	...
	// #region Knee Model
	// Knee basically has only 1 DoF (pitch), average yaw and roll between knee and
	// hip
	protected void calculateKneeLimits(Quaternion hipBuf, Quaternion kneeBuf, float hipConfidence,
	...
	public void resetSkeletonConfig(SkeletonConfigValue config) {
		...
		case TORSO: // Distance from shoulders to hip (full torso length)
			...
			height = vec.y;
			if (height > 0.5f) { // Reset only if floor level is right, TODO: read floor level from SteamVR if
				// it's not 0
				skeletonConfig.setConfig(SkeletonConfigValue.TORSO,
		...
		case CHEST: // Chest is 57% of the upper body by default (shoulders to chest)
			skeletonConfig.setConfig(SkeletonConfigValue.CHEST,
		...
		case LEGS_LENGTH: // Set legs length to be 5cm above floor level
			...
			if (height > 0.5f) { // Reset only if floor level is right, todo: read floor level from SteamVR if
				// it's not 0
				skeletonConfig.setConfig(SkeletonConfigValue.LEGS_LENGTH,
						height - skeletonConfig.getConfig(SkeletonConfigValue.NECK)
								- skeletonConfig.getConfig(SkeletonConfigValue.TORSO) - 0.05f);
			} else // if floor level is incorrect
			{
		...
		case KNEE_HEIGHT: // Knees are at 55% of the legs by default
			skeletonConfig.setConfig(SkeletonConfigValue.KNEE_HEIGHT,
	...
	Tracker[] getTrackerToReset() {
		return new Tracker[] { trackerPreUpdate(this.neckTracker), trackerPreUpdate(this.chestTracker),
