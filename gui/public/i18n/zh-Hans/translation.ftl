...
settings-serial-enter_pairing = 开始配对
settings-serial-exit_pairing = 退出配对
settings-serial-calibrate = 校准
settings-serial-six_side_calibrate = 六面校准
settings-serial-dfu = 进入 DFU 模式
...
tracking_checklist-STEAMVR_DISCONNECTED-driver_blocked-desc = 由于之前发生的 SteamVR 崩溃，SlimeVR 驱动已经被 SteamVR 屏蔽。
tracking_checklist-STEAMVR_DISCONNECTED-driver_disabled-desc = SlimeVR 驱动已在 SteamVR 设置中被屏蔽。
tracking_checklist-STEAMVR_DISCONNECTED-driver_not_installed-desc = SlimeVR 驱动未安装。
tracking_checklist-STEAMVR_DISCONNECTED-open = 启动 SteamVR
tracking_checklist-STEAMVR_DISCONNECTED-enable = 启用驱动
...
tracking_checklist-STANDABLE_INSTALLED = Standable 程序已安装
tracking_checklist-STANDABLE_INSTALLED-desc =
    Standable 与 SlimeVR 共同使用时易产生追踪问题。为保证 SlimeVR 使用流畅，建议从 Steam 中完全卸载 Standable 程序。
    在卸载 Standable 前，需要先关闭 SteamVR。
