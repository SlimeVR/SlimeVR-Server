...
settings-stay_aligned-description = Stay Aligned 會逐漸調整追蹤器以對齊放鬆的姿勢，進以減緩偏移。
...
settings-stay_aligned-hide_yaw_correction-label = 隱藏調整效果（可與關閉 Stay Aligned 的效果對比）
...
onboarding-stay_aligned-description = 設定 Stay Aligned 讓追蹤器保持對齊狀態。
...
onboarding-stay_aligned-put_trackers_on-trackers_warning = 你目前已連接與分配的追蹤器少於 5 個，Stay Aligned 需要 5 個以上的追蹤器才能正常運作。
onboarding-stay_aligned-put_trackers_on-next = 我所有的追蹤器都戴好了
onboarding-stay_aligned-verify_mounting-title = 確認追蹤器的配戴狀態
onboarding-stay_aligned-verify_mounting-step-0 = Stay Aligned 需要良好的追蹤器配戴固定，否則體驗會不如預期。
...
onboarding-stay_aligned-done-description = Stay Aligned 已設定完成。
onboarding-stay_aligned-done-description-2 = 設定完成。若需要重新校正姿勢，你可以重新開始校正流程。
