...
export function useManualProportions({ type }: { type: 'linear' | 'ratio' }): {
  ...
  useRPCPacket(RpcMessage.SkeletonProportionsResponse, (data: SkeletonProportionsResponseT) => {
    setSkeleton(data);
  });

  useEffect(() => {
    sendRPCPacket(RpcMessage.SkeletonProportionsRequest, new SkeletonProportionsRequestT());
  }, []);
