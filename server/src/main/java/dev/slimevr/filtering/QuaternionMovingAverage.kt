    ...
    fun update() {
        if (type === TrackerFilters.PREDICTION) {
            if (rotBuffer!!.size > 0) {
                ...
                rotBuffer!!.forEach(latestQuaternion::multLocal)

                // Slerps the target rotation to that predicted rotation by
                // a certain factor.
                filteredQuaternion
                    .slerpLocal(latestQuaternion, predictFactor * fpsTimer.timePerFrame)
            }
        }
        if (type === TrackerFilters.SMOOTHING) {
            // Calculate the slerp factor and limit it to 1 max
            smoothingCounter++
            val amt = Math.min(smoothFactor * fpsTimer.timePerFrame * smoothingCounter, 1f)
