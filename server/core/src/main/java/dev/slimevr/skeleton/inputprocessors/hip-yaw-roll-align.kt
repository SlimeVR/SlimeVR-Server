...
class HipYawRollAlignInputProcessor(val settings: Settings) : SkeletonInputProcessor {
	override fun process(mutableInputSkeleton: InputSkeleton, skeletonHeight: Float) {
		val hipBone = mutableInputSkeleton[BodyPart.HIP.boneId] ?: return
		if (hipBone.isRotationActive) return
		val leftUpperLegBone = mutableInputSkeleton[BodyPart.LEFT_UPPER_LEG.boneId] ?: return
		val rightUpperLegBone = mutableInputSkeleton[BodyPart.RIGHT_UPPER_LEG.boneId] ?: return
		...
		mutableInputSkeleton[BodyPart.HIP.boneId] = hipBone.copy(rotation = hipBone.rotation.interpQ(alignedRotation, ratio))
	}
}
