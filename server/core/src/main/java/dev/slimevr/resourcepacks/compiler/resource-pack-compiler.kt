...
fun compileResourcePacks(catalog: ResourcePackCatalog): CompiledSkeleton {
	...
	for (contribution in allContributions) {
		...
		definition.inputs?.vrchat?.let { input ->
			reportDuplicateBoneKey(vrchatInputAddresses, input.address, boneId, "VRChat input address", registry, contribution.origin(BoneField.InputsVrchat), diagnostics)
		}
		bones[boneId] = CompiledBone(
			tailOffset = definition.tailOffset?.let { compileOffset(it, contribution.origin(BoneField.TailOffset), proportionsByKey, diagnostics) },
			headOffset = definition.headOffset?.let { compileOffset(it, contribution.origin(BoneField.HeadOffset), proportionsByKey, diagnostics) },
			constraint = definition.constraint?.let { compileConstraint(it) },
			mirror = definition.mirror?.let { resolveBoneKey(it, contribution.origin(BoneField.Mirror), "mirror", registry, diagnostics) },
			candidateSources = definition.candidateSources?.map { resolveBoneKey(it, contribution.origin(BoneField.CandidateSources), "candidateSources", registry, diagnostics) },
			batterySources = definition.batterySources?.map { resolveBoneKey(it, contribution.origin(BoneField.BatterySources), "batterySources", registry, diagnostics) },
