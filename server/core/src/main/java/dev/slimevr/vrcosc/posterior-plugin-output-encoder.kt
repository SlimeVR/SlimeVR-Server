...
private val posteriorOutputState = PosteriorOutputState()

internal fun buildPosteriorMessages(
	bones: Map<BodyPart, BoneState>,
): List<OscContent> =
	posteriorOutputState.buildMessages(bones)

private class PosteriorOutputState {
	...
	private fun getSharedVerticalAccelerationY(
		leftPosterior: BoneState?,
		rightPosterior: BoneState?,
	): Float? =
		when {
			leftPosterior != null &&
				rightPosterior != null ->
				(
					leftPosterior.acceleration.y +
						rightPosterior.acceleration.y
					) * 0.5f
