...
object AdjustTrackerYaw {
	...
	fun adjust(
		...
		tracker.stayAligned.yawErrors = YawErrors()

		if (tracker.stayAligned.lockedRotation != null) {
			adjustLockedTracker(tracker, trackers, yawCorrection)
		} else if (centerYaw != null && relaxedPose != null) {
			adjustMovingTracker(tracker, trackers, yawCorrection, centerYaw, relaxedPose)
		}
	}
	...
	private fun adjustMovingTracker(
		...
		adjustByError(tracker, yawCorrection) {
			YawErrors().apply {
				centerError =
					trackers.visit(tracker, CenterErrorVisitor(centerYaw, relaxedPose))
						?: Angle.ZERO

				neighborError =
					trackers.visit(tracker, NeighborErrorVisitor(relaxedPose))
						?: Angle.ZERO
			}
		}
	}
	...
	private fun gradient(errors: YawErrors, base: YawErrors) =
		(abs(errors.lockedError) - abs(base.lockedError)) * YAW_ERRORS_LOCKED_ERROR_WEIGHT +
			(abs(errors.centerError) - abs(base.centerError)) * YAW_ERRORS_CENTER_ERROR_WEIGHT +
			(abs(errors.neighborError) - abs(base.neighborError)) * YAW_ERRORS_NEIGHBOR_ERROR_WEIGHT
}
