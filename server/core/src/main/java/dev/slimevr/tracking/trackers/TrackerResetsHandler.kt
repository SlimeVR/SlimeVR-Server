...
class TrackerResetsHandler(val tracker: Tracker) {
    ...
    /**
     * Gyro fix is set by full reset. This sets the current y rotation to 0, correcting for initial
     * yaw rotation and the rotation incurred by mounting orientation. This is a local offset in
     * rotation and does not affect the axes of rotation.
     *
     * This rotation is only used to compute [attachmentFix], otherwise [yawFix] would correct for
     * the same rotation.
     */
    private var gyroFix = Quaternion.IDENTITY

    /**
     * Attachment fix is set by full reset. This sets the current x and z rotations to 0, correcting
     * for initial pitch and roll rotation. This is a global offset in rotation and affects the axes
     * of rotation.
     *
     * This effectively sets the rotation at the moment of a full reset to be zero-reference in the
     * x and z axes.
     */
    private var attachmentFix = Quaternion.IDENTITY

    /**
     * Mounting rotation fix is set by mounting reset. This corrects for the mounting orientation,
     * then the inverse is used to correct for the rotation incurred. This value is computed after
     * [yawFix], but takes effect before [yawFix]. This affects the axes of rotation, but does not
     * incur an offset in rotation.
     *
     * This rotation is done in addition to [mountingOrientation] as to not interfere with the
     * functionality of manual mounting orientation. This effectively sets the rotation at the
     * moment of a mounting reset to be zero-reference in the y-axis. If no mounting reset is done,
     * then this rotation will not be used and only [mountingOrientation] will apply.
     */
    var mountRotFix = Quaternion.IDENTITY
        private set

    /**
     * Yaw fix is set by yaw reset. This sets the current y rotation to match the provided
     * reference, correlating the tracker to the provided frame of reference. This is a local offset
     * in rotation and does not affect the axes of rotation.
     *
     * This effectively aligns the current yaw rotation to the head tracker's yaw rotation.
     */
    private var yawFix = Quaternion.IDENTITY

    /**
     * Constraint fix is set by skeleton constraints. This corrects for any yaw rotation that
     * violates the skeleton constraints. This is a local offset in rotation and does not affect the
     * axes of rotation.
     */
    ...
    /**
     * T-Pose down fix is set by full reset. This corrects for the pitch of the rotation assuming a
     * t-pose reference, adjusting to match our expected i-pose reference. This is a global offset
     * in rotation and affects the axes of rotation.
     */
    ...
    /**
     * Checks for compensateDrift, allowDriftCompensation, and if a computed head tracker exists.
     */
    ...
    /**
     * Takes a rotation and adjusts it to resets, mounting, and drift compensation, with the HMD as
     * the reference.
     */
    fun getReferenceAdjustedDriftRotationFrom(rotation: Quaternion): Quaternion =
            adjustToDrift(adjustToYawResetSmoothing(adjustToReference(rotation)))

    /**
     * Takes a rotation and adjusts it to resets and mounting, with the identity Quaternion as the
     * reference.
     */
    ...
    fun getReferenceAdjustedAccel(rawRot: Quaternion, accel: Vector3): Vector3 =
            (adjustToReference(rawRot) / yawFix).sandwich(accel)

    /**
     * Converts raw or filtered rotation into reference- and mounting-reset-adjusted by applying
     * quaternions produced after full reset, yaw rest and mounting reset
     */
    ...
    /**
     * Converts raw or filtered rotation into zero-reference-adjusted by applying quaternions
     * produced after full reset and yaw reset only
     */
    ...
    /** Adjust the given rotation for drift compensation if enabled, and returns it */
    ...
    /**
     * Apply yaw reset smoothing to quaternion rotated to new yaw fix and returns smoothed
     * quaternion
     */
    ...
    /**
     * Reset the tracker so that its current rotation is counted as (0, HMD Yaw, 0). This allows the
     * tracker to be strapped to body at any pitch and roll.
     */
    fun resetFull(reference: Quaternion) {
        ...
        tposeDownFix =
                if (((isLeftArmTracker() || isLeftFingerTracker()) &&
                                armsResetMode == ArmsResetModes.TPOSE_DOWN)
                ) {
                    EulerAngles(EulerOrder.YZX, 0f, 0f, -FastMath.HALF_PI).toQuaternion()
                } else if (((isRightArmTracker() || isRightFingerTracker()) &&
                                armsResetMode == ArmsResetModes.TPOSE_DOWN)
                ) {
                    EulerAngles(EulerOrder.YZX, 0f, 0f, FastMath.HALF_PI).toQuaternion()
                } else {
        ...
        if (tracker.allowMounting ||
                        (tracker.trackerPosition == TrackerPosition.HEAD && !tracker.isHmd)
        ) {
            gyroFix =
        ...
        // Attachment fix
        attachmentFix =
                if (tracker.trackerPosition == TrackerPosition.HEAD && tracker.isHmd) {
                    if (resetHmdPitch) {
                        ...
                        val rotBuf =
                                getYawQuaternion(tracker.getRawRotation()).inv() *
    ...
    /**
     * Reset the tracker so that its current yaw rotation is aligned with the HMD's Yaw. This allows
     * the tracker to have yaw independent of the HMD. Tracker should still report yaw as if it was
     * mounted facing HMD, mounting position should be corrected in the source.
     */
    fun resetYaw(reference: Quaternion) {
        // TODO HMD doesn't get yaw reset, which makes it so tracker.resetFilteringQuats() doesn't
        // get called
    ...
    /**
     * Perform the math to align the tracker to go forward and stores it in mountRotFix, and adjusts
     * yawFix
     */
    fun resetMounting(reference: Quaternion) {
        ...
        val isLowerArmBack =
                armsResetMode == ArmsResetModes.BACK &&
                        (isLeftLowerArmTracker() || isRightLowerArmTracker())
        val isArmForward =
                armsResetMode == ArmsResetModes.FORWARD &&
                        (isLeftArmTracker() || isRightArmTracker())
    ...
    private fun getYawQuaternion(rot: Quaternion): Quaternion =
            EulerAngles(EulerOrder.YZX, 0f, rot.toEulerAngles(EulerOrder.YZX).y, 0f)
                    .toQuaternion()
                    .twinNearest(rot)
    ...
    /**
     * Calculates drift since last reset and store the data related to it in driftQuat and
     * timeAtLastReset
     */
        ...
        if (driftCompensationEnabled) {
            ...
            if (driftSince > 0 && System.currentTimeMillis() - timeAtLastReset > DRIFT_COOLDOWN_MS
                ...
                for (i in driftWeights.size - 1 downTo 1) {
                    ...
                    driftWeights[i - 1] =
                            driftWeights[i - 1] - driftWeights[i - 1] / driftWeights.size
                }
    ...
    /** Calculates and returns the averaged Quaternion from the given Quaternions and weights. */
    ...
    private fun isThighTracker(): Boolean {
        tracker.trackerPosition?.let {
            return it == TrackerPosition.LEFT_UPPER_LEG || it == TrackerPosition.RIGHT_UPPER_LEG
        }
        return false
    }
    ...
    private fun isLeftLowerArmTracker(): Boolean {
        tracker.trackerPosition?.let {
            return it == TrackerPosition.LEFT_LOWER_ARM || it == TrackerPosition.LEFT_HAND
        }
        return false
    }
    ...
    private fun isRightLowerArmTracker(): Boolean {
        tracker.trackerPosition?.let {
            return it == TrackerPosition.RIGHT_LOWER_ARM || it == TrackerPosition.RIGHT_HAND
        }
        return false
    }
    ...
    private fun isFootTracker(): Boolean {
        tracker.trackerPosition?.let {
            return it == TrackerPosition.LEFT_FOOT || it == TrackerPosition.RIGHT_FOOT
        }
        return false
    }
