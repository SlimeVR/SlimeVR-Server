...
class TapDetection(val skeleton: HumanSkeleton, val trackerToWatch: Tracker, val accelThreshold: Int, val tapToComplete: Int, val onTapCompleted: () -> Unit) {
	...
	private fun isUserStatic(trackerToExclude: Tracker): Boolean {
		...
		val numTrackersAboveThreshold = trackersToCheck.count {
			it != null && it != trackerToExclude && it.getAcceleration().lenSq() > ALLOWED_BODY_ACCEL_SQUARED
		}
		return numTrackersAboveThreshold < accelThreshold
	}
