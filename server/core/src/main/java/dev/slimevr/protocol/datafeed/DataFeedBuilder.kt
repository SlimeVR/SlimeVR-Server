...
object DataFeedBuilder {
    ...
    fun createTrackerId(fbb: FlatBufferBuilder, tracker: Tracker): Int {
        ...
        if (tracker.device != null) TrackerId.addDeviceId(fbb, DeviceId.createDeviceId(fbb, tracker.device.id))

        return TrackerId.endTrackerId(fbb)
    }

    fun createVec3(fbb: FlatBufferBuilder, vec: Vector3): Int {
        return Vec3f
            .createVec3f(
                fbb,
                vec.x,
    ...
    fun createQuat(fbb: FlatBufferBuilder, quaternion: Quaternion): Int {
        return Quat
            .createQuat(
                fbb,
                quaternion.x,
    ...
    fun createTrackerInfos(
        ...
        if (tracker.trackerPosition != null) TrackerInfo.addBodyPart(fbb, tracker.trackerPosition!!.bodyPart)
    ...
    fun createTrackerPosition(fbb: FlatBufferBuilder, tracker: Tracker): Int {
        return createVec3(fbb, tracker.position)
    }

    fun createTrackerRotation(fbb: FlatBufferBuilder, tracker: Tracker): Int {
        return createQuat(fbb, tracker.getRawRotation())
    }

    fun createTrackerAcceleration(fbb: FlatBufferBuilder, tracker: Tracker): Int {
        return createVec3(fbb, tracker.getAcceleration())
    }

    fun createTrackerMagneticVector(fbb: FlatBufferBuilder, tracker: Tracker): Int {
    ...
    fun createTrackerData(
        ...
        if (mask.stayAligned) {
            stayAlignedOffset = createTrackerStayAlignedTracker(fbb, tracker.stayAligned)
        }
        ...
        if (mask.position && tracker.hasPosition) TrackerData.addPosition(fbb, createTrackerPosition(fbb, tracker))
        if (mask.rotation && tracker.hasRotation) TrackerData.addRotation(fbb, createTrackerRotation(fbb, tracker))
        if (mask.linearAcceleration && tracker.hasAcceleration) TrackerData
        ...
        if (mask.temp) {
            ...
            if (trackerTemperatureOffset != 0) TrackerData.addTemp(fbb, trackerTemperatureOffset)
        }
    ...
    fun createTrackersData(
        ...
        trackersOffsets.forEach(Consumer { offset: Int -> DeviceData.addTrackers(fbb,
			offset
		) })
    ...
    fun createSyntheticTrackersData(
        ...
        trackerOffsets.forEach((Consumer { tracker: Int -> DataFeedUpdate.addSyntheticTrackers(fbb, tracker) }))
        return fbb.endVector()
    }
    ...
    fun createDevicesData(
        ...
        for (i in devices.indices) {
            ...
            devicesDataOffsets[i] = createDeviceData(fbb, device.id, deviceDataMaskT, device)
        }
