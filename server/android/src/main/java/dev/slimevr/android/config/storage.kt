package dev.slimevr.android.config
...
class AndroidConfigStorage(private val root: File) : ConfigStorage {
	...
	override suspend fun openTextFile(path: String): TextFileHandle = withContext(Dispatchers.IO) {
		val file = resolve(path)
		file.parentFile?.mkdirs()
		AndroidRandomAccessTextFileHandle(RandomAccessFile(file, "rw").also { handle -> handle.setLength(0L) })
	}
...
private class AndroidRandomAccessTextFileHandle(
