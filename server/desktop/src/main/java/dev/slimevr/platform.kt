...
fun getSocketDirectory(): String {
	...
	if (CURRENT_PLATFORM == Platform.LINUX) {
		...
		if (isPressureVessel) {
			val dir = System.getenv("XDG_DATA_HOME")?.let { Path(it, SLIMEVR_IDENTIFIER).toString() }
...
fun resolveConfigDirectory(): Path? {
	...
	return when (CURRENT_PLATFORM) {
		...
		Platform.LINUX -> {
			...
			if (xdg != null) {
				Path(xdg, SLIMEVR_IDENTIFIER)
			} else if (home != null) {
				Path(home, ".config", SLIMEVR_IDENTIFIER)
			} else {
				null
			}
		}
...
fun resolveLogDirectory(): Path? {
	...
	return when (CURRENT_PLATFORM) {
		...
		Platform.LINUX -> {
			...
			if (xdg != null) {
				Path(xdg, SLIMEVR_IDENTIFIER, "logs")
			} else if (home != null) {
				Path(home, ".local", "share", SLIMEVR_IDENTIFIER, "logs")
			} else {
				null
			}
		}
